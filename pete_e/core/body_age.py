...
def calculate_body_age(withings_history: List[Dict], apple_history: List[Dict], profile: Dict) -> Dict[str, Any]:
    "#""Compute body age using rolling 7-day averages."""
    today = date.today().isoformat()

    dates = sorted({r.get("date") for r in withings_history + apple_history if rget("date")})
    if not dates:
        return {"date": today, "error": "No input data"}
    dates = dates[-7:]

    def avg_from(fn):
        vals = [to_float(fn(d)) for d in withings_history + apple_history if d in dates]
        return average(vals)
